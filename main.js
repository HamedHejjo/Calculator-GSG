...
function isCurrencySentence(curr) {
  return (
    curr === "Shekels converted to Dollars" ||
    curr === "Dollars converted to Shekels" ||
    curr === "Shekels converted to Euros" ||
    curr === "Euros converted to Shekels"
  );
}
...
keys.addEventListener("click", (e) => {
  ...
  let lastClicked = equation.substring(
    equation.length - 2,
    equation.length - 1
  );
  let lastClickedMod = equation.substring(
    equation.length - 4,
    equation.length - 1
  );
  ...
  if (e.target.matches("button")) {
    ...
    if (equation === "0" && clickedKey === "0") {
      // don't repeat the zero
      display.textContent = "0";
    }
    if (clickedKey === "C") {
      // clear the result and the display
      display.textContent = "0";
    ...
    if (clickedKey === "CE") {
      if (
        (isOperator(lastClicked) || lastClicked === "d") &&
        lastClicked !== "="
      ) {
        // don't clear the operand
        display.textContent = equation;
      } else if (equation === "0") {
        // to not add CE in display and to not remove the zero
        display.textContent = "0";
      } else if (equation.includes("=")) {
        // doing like c (clear)
        display.textContent = "0";
      } else if (isCurrencySentence(equation)) {
        // to remove the currency converter
        display.textContent = "0";
      } else if (equation.length == 1) {
        // don't clear the zero
        display.textContent = "0";
        ...
        ); //the length is 3 (2 for CE and 1 for the last number)
      }
    }

    if (
      equation.substring(equation.length - 2, equation.length) === "." &&
      clickedKey === "."
    ) {
      display.textContent = equation.substring(
        0,
    ...
    if (equation === "0" && isOperator(clickedKey)) {
      // if you start with operand
      display.textContent = "0 " + clickedKey + " ";
    }

    if (
      (isOperator(lastClicked) || isOperator(lastClickedMod)) &&
      isOperator(clickedKey)
    ) {
      // to prevent repaeat last operator
      // i need to pass convert mod to + - / *  and pass convert + - * / to mod
      if (clickedKey !== "mod" && !equation.includes("mo" + lastClicked)) {
        // if repeat + - * /
        display.textContent =
          display.textContent.substring(0, display.textContent.length - 5) +
          clickedKey +
          " ";
      } else if (
        clickedKey === "mod" &&
        !equation.includes("mo" + lastClicked)
      ) {
        // to change + - * / to mod
        display.textContent =
          display.textContent.substring(0, display.textContent.length - 7) +
          clickedKey +
          " ";

        // you can mix the bottom condition with the above condition
        //but im my opinion like this will be more clear
      } else if (clickedKey !== "mod") {
        // to change mod to + - * /
        display.textContent =
          display.textContent.substring(0, display.textContent.length - 7) +
          clickedKey +
        ...
        // if repeat mod
        display.textContent =
          display.textContent.substring(0, display.textContent.length - 9) +
          clickedKey +
          " ";
      }
    }
    ...
    if (clickedKey === "=") {
      ...
      let checkCorrection = equation.slice(
    ...
    let exactCurrency = display.textContent
      .substring(0, display.textContent.length - 2)
      .trim(); // to get the sentence without the operand
